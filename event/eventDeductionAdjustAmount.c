...
#endif
static void event_class_deduction_adjust_display(EVENT_INTERFACE *pIntf)
{
    EVENT_CLASS_DEDUCTION_ADJUST_AMOUNT *this_ptr;
	
    this_ptr=(EVENT_CLASS_DEDUCTION_ADJUST_AMOUNT *)pIntf;
	printf("event_deduction_adjust_display:%d,%s\n",pIntf->mei_type,TM_to_asc_simple(pIntf->mei_time));
...
{
    EVENT_CLASS_DEDUCTION_ADJUST_AMOUNT *this_ptr;
	event_class_base_init_helper(pIntf, ME_DEDUCTION_ADJUST_AMOUNT, time,event_class_deduction_adjust_display);
